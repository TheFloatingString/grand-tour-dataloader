...
def _get_camera_sensors(positions: Optional[Set[str]]) -> Set[SensorType]:
...
def _build_allow_patterns(mission_dir: str, sensors: Set[SensorType]) -> List[str]:
    """Build HuggingFace download patterns for sensors."""
    patterns = [
        f"{mission_dir}/*.yaml",
        f"{mission_dir}/data/.zgroup",
    ]
    for sensor in sensors:
        patterns.append(f"{mission_dir}/data/{sensor.value}*")
    return patterns


def _collect_downloaded_files(
    cache_path: Path, mission_dir: str, sensors: Set[SensorType]
) -> Dict[str, List[Path]]:
    """Collect downloaded files organized by sensor."""
    sensor_files: Dict[str, List[Path]] = {sensor.value: [] for sensor in sensors}

    data_path = cache_path / mission_dir / "data"
    ...
    for file_path in data_path.iterdir():
        if file_path.is_file():
            for sensor in sensors:
                if file_path.name.startswith(sensor.value):
                    sensor_files[sensor.value].append(file_path)
                    break

    return sensor_files


def _filter_empty_sensors(sensor_files: Dict[str, List[Path]]) -> Dict[str, List[Path]]:
    """Remove sensors with no files."""
    return {name: files for name, files in sensor_files.items() if files}
...
class GrandTourDataLoader:
    ...
    def load(self) -> Dict[str, List[Path]]:
        ...
        Returns:
            Dictionary mapping sensor names to file paths.
        """
        ...
        allow_patterns = _build_allow_patterns(self.mission_dir, sensors_to_load)
        ...
        sensor_files = _collect_downloaded_files(
            self._cache_path, self.mission_dir, sensors_to_load
        )
        return _filter_empty_sensors(sensor_files)

    def extract_tar_contents(
        self, tar_path: Path, extract_path: Optional[Path] = None
    ) -> Path:
        ...
        if extract_path is None:
            ...
            extract_path = self.cache_dir / "extracted" / tar_path.name
